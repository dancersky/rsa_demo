...
由于二进制数据打印出来也不容易阅读，现在我们把加密的二进制密文转为base64的编码格式，
base64是六个bit组成一个单元表示一个字符，那么就是说三个字节的二进制数据24个bit就相
当于4个单元的base64，即四个字节。base64编码就是为了数据传输易读。所以一般加密后密文
转为base64格式，但解密时要将base64转换为原来的二进制类型密文。
